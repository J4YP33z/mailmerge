...
for i, col in enumerate(rawData[0:]):
  if (lastNameProcessed.upper() == col[0].upper() and lastCountryProcessed.upper() == col[15].upper()):
    lastRecordedIndex = i
...
tmpStr = (config['CONFIG']['RawData']).strip() + " "+ rawData[-1][0] + " " + rawData[-1][15]
...
# write headers
for i, header in enumerate(["NAME", "EMAIL", "TRACKING NO.", "PHONE", "ADDRESS", "CITY", "STATE", "POSCODE", "COUNTRY", "ORDERS", "STATUS", "COST"]):
  SG_WS.write(0, i, header)
  MY_WS.write(0, i, header)
  PH_WS.write(0, i, header)
  OTHERS_WS.write(0, i, header)

for i, header in enumerate(["Pick-up Account Number","Shipment Order ID","Shipping Service Code","Consignee Name","Address Line 1","Address Line 2","Address Line 3","City","State (M)","Postal Code (M)","Destination Country Code","Phone Number","Email Address","Shipment Weight (g)","Currency Code","Total Declared Value","Incoterm","Shipment Description","Content Description","Content Export Description","Content Unit Price","Content Origin Country","Content Quantity","Content Code","Content Indicator","Remarks"]):
...
for col in rawData[0:]:
  ...
  if col[15] == "Singapore":
    ...
    for k, content in enumerate(["5345221", shipmentOrderID, shippingServiceCode, col[0][:30], col[11], "", "", col[6], col[7], col[9], countryCode, col[10], col[3], weight, "MYR", declaredValue, incoterm, "educational book, perfect bound book", "educational book, perfect bound book", "", declaredValue, "MY", 1, shipmentOrderID, "", ""]):
      DHL_WS.write(DHLindex, k, content)
    DHLindex += 1
    startingNumber += 1

  for j, content in enumerate([col[0], col[3], shipmentOrderID, col[10], col[11], col[13], col[14], col[16], col[15], col[17], "", ""]):
